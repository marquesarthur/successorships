...
All computers used Firefox Aurora nightly build ({\ttfamily{50.0a2}}) and the three researchers acted as users of the queue application.
...
For each one of the events presented in the Table~\ref{tbl:eval:experiment_summary}, we are interested in measuring the amount of time it took from when the event occurred until when devices acknowledged the event and responded executing expected function in response to that event. 
...
As an example, whenever a running server disconnects, our instrumented application logs an event of ``{\ttfamily{disconnecting}}'', which is followed by a successor device responding with a ``{\ttfamily{become\_server\_begin}}'' and either a ``{\ttfamily{become\_server\_end}}'' or ``{\ttfamily{become\_server\_error}}'' after the function's execution. This events are chronologically sorted, as depicted in the Listing~\ref{lst:events}, and we compute the elapsed time between them.
